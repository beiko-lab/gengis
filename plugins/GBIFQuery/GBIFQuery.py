...
class GBIFQuery(GBIFQueryLayout):
	...
	def OnAddData(self,event):
		if (len(self.__obs__) > 0):
			...
			OUTLArray.insert(0,"Site ID,Latitude,Longitude,Richness,Cell ID,Taxon")
	...
	def OnExportData(self,event):
		if (len(self.__obs__) > 0):
			...
			if dlg.ShowModal()==wx.ID_OK:
				...
				OUTLText, OUTSText = self.GBIFGeneric.GETTEXT(self.__obs__,self.__conversions__)
				self.GBIFGeneric.WRITEEXPORT(OUTLfile,OUTLText,"Site ID,Latitude,Longitude,Richness,Cell ID,Taxon\n")
