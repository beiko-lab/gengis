...
//
// Modified by Donovan H Parks (August, 2011) for use in GenGIS
/////////////////////////////////////////////////////////////////////////////

#include "../core/Precompiled.hpp"

#include "MathPlot.hpp"
...
void mpInfoCoords::UpdateInfo(mpWindow& w, wxEvent& event)
{
	if (event.GetEventType() == wxEVT_MOTION) 
	{
		...
		m_content.Printf(wxT("Angle: %.1f"), w.p2x(mouseX));
	}
}
...
/*				if ((p-tx/2-tmp) > 64) { // Problem about non-regular axis labels
		...
		switch (m_flags) {
			case mpALIGN_BORDER_BOTTOM:
				dc.DrawText( m_name, std::max(0, w.GetMarginLeft() + (w.GetScrX() - tx - w.GetMarginLeft() - w.GetMarginRight()) / 2), orgy - 8 - ty - labelH);
			break;
			case mpALIGN_BOTTOM: {
				if ((!m_drawOutsideMargins) && (w.GetMarginBottom() > (ty + labelH + 8))) {
					dc.DrawText( m_name, std::max(0, w.GetMarginLeft() + (w.GetScrX() - tx - w.GetMarginLeft() - w.GetMarginRight()) / 2), orgy + 6 + labelH);
				} else {
					dc.DrawText( m_name, std::max(0, w.GetMarginLeft() + (w.GetScrX() - tx - w.GetMarginLeft() - w.GetMarginRight()) / 2), orgy - 4 - ty);
				}
			} break;
			case mpALIGN_CENTER:
...
void mpScaleY::Plot(wxDC & dc, mpWindow & w)
{
    ...
    // Draw line
		dc.DrawLine( orgx, w.GetMarginTop(), orgx, extend - w.GetMarginBottom());
...
//					float a = w.GetScrY();
//					float b = w.GetMarginBottom();
//					float c = w.GetMarginTop();
					dc.DrawRotatedText( m_name, orgx - 10 - labelW - ty, std::max(0, (w.GetScrY() + tx - w.GetMarginBottom() - w.GetMarginTop()) / 2 + w.GetMarginTop()), 90);
				} else {
...
BEGIN_EVENT_TABLE(mpWindow, wxWindow)
    ...
    EVT_LEFT_UP( mpWindow::OnMouseLeftUp)

    EVT_MENU(mpSAVE_PLOT, mpWindow::OnSavePlot)
END_EVENT_TABLE()
    ...
    : wxWindow( parent, id, pos, size, flag, wxT("mathplot") )
{
    ...
    m_popmenu.Append(mpSAVE_PLOT, _("Save plot"), _("Save plot to file"));
...
void mpWindow::OnMouseLeftUp(wxMouseEvent &event)
{
	float angle = p2x(event.GetX());
	m_sigPlotClicked(angle);
...
void mpWindow::OnSavePlot(wxCommandEvent& WXUNUSED(event))
{
	...
	SaveScreenshot(saveDlg->GetPath(), wxBITMAP_TYPE_PNG);
}
...
void mpText::Plot(wxDC & dc, mpWindow & w)
{
	...
	/*    dc.DrawText( GetName(),
		...
		{
			dc.DrawText(GetName(), 5, w.GetScrY() - th - 5);
		}
	}
}
