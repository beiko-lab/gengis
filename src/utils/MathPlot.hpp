...
#ifndef _GENGIS_MP_MATHPLOT_H_
#define _GENGIS_MP_MATHPLOT_H_
...
/** Canvas for plotting mpLayer implementations.
    ...
    This class defines a 2D plot canvas. Any number of mpLayer implementations (scale rulers, 
		function plots, ...) can be attached using mpWindow::AddLayer.
...
{
public:
	/** Function signature for plot clicked signal. */
	typedef boost::signal<void (float angle)> SigPlotClicked;
...
public:
	 ...
	 */
	void SignalPlotClicked(const SigPlotClicked::slot_type& slot) { m_sigPlotClicked.connect(slot); }
...
protected:
	/** Signal that the plot has been clicked. */
	SigPlotClicked m_sigPlotClicked;
...
public:
    /** Default constructor.
      */
    mpPolygon( const wxString & layerName = wxEmptyString )
    {
        m_continuous = true;
        m_name = layerName;
    }
